...
void CYouMeTalkJsWrapper::onPcmData(int channelNum, int samplingRateHz, int bytesPerSample, void* data, int dataSizeInByte){
    //todo:暂时不暴露
}
...
bool js_cocos2dx_extension_talk_SetWhiteUserList(JSContext *cx, uint32_t argc, JS::Value *vp) {
    ...
    if (argc == 2)
    {
        ...
        int iErrorcode = IYouMeVoiceEngine::getInstance()->setWhiteUserList( strChannel.c_str(), vecWhiteUserList);
        args.rval().set(int32_to_jsval(cx, iErrorcode));
